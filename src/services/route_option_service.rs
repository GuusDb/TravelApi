...
impl RouteOptionService {
    pub fn get_route_options(
        ...
        info!("Fetching route options for travel plan ID: {} for user: {}", plan_id, user_id);
        ...
        match RouteOption::find_by_travel_plan_id(conn, plan_id) {
            Ok(routes) => {
                ...
                info!("Found {} route options for travel plan ID: {}", routes_with_pois.len(), plan_id);
                Ok(routes_with_pois)
            }
            Err(e) => {
    ...
    pub fn generate_route_options(
        ...
        match RouteOption::generate_random_options(conn, plan_id, count) {
            Ok(routes) => {
                ...
                info!("Generated {} route options with points of interest for travel plan ID: {}", 
                      routes_with_pois.len(), plan_id);
                Ok(routes_with_pois)
            }
            Err(e) => {
    ...
    pub fn get_route_option_by_id(
        ...
        match RouteOption::find_by_id(conn, route_id) {
            Ok(Some(route)) => {
                ...
                match PointOfInterest::find_by_route_option_id(conn, &route.id) {
                    Ok(pois) => {
                        info!("Found route option with ID: {} with {} points of interest", 
                              route.id, pois.len());
